 ...
 * before we adopt it" supply-chain policy deterministically, at a layer we
 ...
 * walks can merge while each version is only a few days old, well inside the
 * configured window.
 *
 ...
 * on every update — a full-metadata storm that causes multi-minute/hour
 ...
 *            pnpm-lock.yaml is read via `git show <baseRef>:pnpm-lock.yaml`; the
 *            head lockfile is read from ./pnpm-lock.yaml in the working tree.
 *            Only versions present in head but not base are checked.
 *
 ...
 * fetch failures are warned and skipped (fail-open) so a flaky registry does not
 * produce a spurious red build; a confirmed too-young version always fails.
 */
...
/**
 * Collect the `name@version` keys from the top-level `packages:` block of a pnpm
 * v9 lockfile. That block lists each resolved package canonically, without the
 * peer-dependency suffixes that appear in `snapshots:`, so its keys are exactly
 * the `name@version` pairs we want. Returns a Set of "name@version".
 */
