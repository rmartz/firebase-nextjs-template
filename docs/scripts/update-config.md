...
- `KEY=value` — one or more public config pairs to set. Never pass secrets this way; they leak into shell history and `ps` output. Manage secrets with the `envctl` CLI (in development).
...
This script only writes and validates the local YAML. Pushing the values to Vercel is handled separately by the `envctl` CLI (in development).
